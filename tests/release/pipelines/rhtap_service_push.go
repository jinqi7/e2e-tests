...
var _ = framework.ReleasePipelinesSuiteDescribe("e2e tests for rhtap-service-push pipeline", Label("release-pipelines", "rhtap-service-push", "HACBS"), func() {
	...
	BeforeAll(func() {
		...
		//GITHUB_E2E_ORGANIZATION_ENV(MY_GITHUB_ORG)
		owner := utils.GetEnv(constants.GITHUB_E2E_ORGANIZATION_ENV,"hacbs-release-tests")
		githubToken := utils.GetEnv(constants.GITHUB_TOKEN_ENV, "")
		Expect(githubToken).ToNot(BeEmpty())

		gh, err := github.NewGithubClient(githubToken, "redhat-appstudio")
		Expect(err).NotTo(HaveOccurred())

		forkExist, err := gh.CheckIfForkExist("infra-deployments", owner)
		Expect(err).NotTo(HaveOccurred(), "Error when checking fork")

		if !forkExist {
			err := gh.CreateFork("infra-deployments")
			Expect(err).NotTo(HaveOccurred(), "Create fork failed")
		}

		//forked repo format: github.com/$(owner)/infra-deployments
		_, err = gh.MergeUpstream(owner, "infra-deployments", "main")
		Expect(err).NotTo(HaveOccurred(), "Merge upstream failed")
