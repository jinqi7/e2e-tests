			...
			It("verifies that a build PipelineRun is created in dev namespace and succeeds", func() {
				...
				Eventually(func() error {
					...
					if buildPR.GetStatusCondition().GetCondition(apis.ConditionSucceeded).IsTrue() {
						...
						return fmt.Errorf(tekton.GetFailedPipelineRunLogs(devFw.AsKubeDeveloper.ReleaseController.KubeRest(), devFw.AsKubeDeveloper.ReleaseController.KubeInterface(), buildPR))
					}
                                        return nil
