...
func CreateReleaseComponent(f framework.Framework, testNamespace, applicationName, componentRepoName, componentRepoURL, gitRevision, contextDir, dockerFilePath string, buildPipelineBundle map[string]string) (*appservice.Component, string, string) {
	...
	if buildPipelineBundle["build.appstudio.openshift.io/pipeline"] != string(`{"name": "fbc-builder", "bundle": "latest"}`) {
		// deal with custom pipeline bundle
...
func CreatePushSnapshot(devFw *framework.Framework, devNamespace, appName, compRepoName, pacBranchName string, pipelineRun *pipeline.PipelineRun, component *appservice.Component) *appservice.Snapshot {
	var prSHA string
	var mergeResult *ghub.PullRequestMergeResult
	var prNumber int
	var err error
	...
	Expect(devFw.AsKubeAdmin.HasController.WaitForComponentPipelineToBeFinished(component, headSHA, devFw.AsKubeAdmin.TektonController, &has.RetryOptions{Retries: 5, Always: true}, pipelineRun)).To(Succeed())
...
func CheckReleaseStatus(releaseCR *releaseApi.Release) error {
	...
	if len(conditions) > 0 {
		for _, c := range conditions {
			GinkgoWriter.Println("Type of condition: %s", c.Type)
			if c.Type == "Released" {
				GinkgoWriter.Println("Status of condition: %s", c.Status)
				if c.Status == "True" {
